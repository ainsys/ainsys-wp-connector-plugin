...
namespace Ansys\connector\WooCommerce;
...
class ainsys_settings {
	...
	static $ainsys_entitis_settings = 'ainsys_entitis_settings';
	...
	static function init() {
		...
		$limit_date   = (int) ainsys_settings::get_option( 'log_until_certain_time' ) ? date( "Y-m-d H:i:s", ainsys_settings::get_option( 'log_until_certain_time' ) ) : '';
		...
		/// Ajax nonce
		self::$nonce_title = 'ansys_admin_menu_nonce';
	...
	public static function admin_enqueue_scripts() {
		...
		if ( isset( $_GET["page"] ) && $_GET["page"] === 'ainsys_connector/includes/ainsys_settings.php' ) {
	...
	private static function get_schema() {
		...
		$table_entitis_settings = $wpdb->prefix . self::$ainsys_entitis_settings;
	...
	static function get_entitis() {
		...
		if ( ainsys_settings::is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
			$entitis['order']   = __( 'Order / fields', AINSYS_CONNECTOR_TEXTDOMAIN );
			$entitis['product'] = __( 'Product / fields', AINSYS_CONNECTOR_TEXTDOMAIN );
			if ( wc_coupons_enabled() ) {
				$entitis['coupons'] = __( 'Coupons / fields', AINSYS_CONNECTOR_TEXTDOMAIN );
			}
		}

		/// Get ACF enteties
		if ( ainsys_settings::is_plugin_active( 'advanced-custom-fields-pro-master/acf.php' ) ) {
			$entitis['acf'] = __( 'ACF  / fields', AINSYS_CONNECTOR_TEXTDOMAIN );
		}
	...
	static function get_user_fields() {
		...
		if ( ainsys_settings::is_plugin_active( 'advanced-custom-fields-pro-master/acf.php' ) ) {
			...
			$acf_fields = self::generate_acf_fields_for_entity( $acf_group );
		}

		return array_merge( $prepered_fields, $acf_fields );
	}
	 ...
	 * @return array
	 */
	static function generate_acf_fields_for_entity( $acf_fields ) {
		if ( empty( $acf_fields ) ) {
			return array();
		}
	...
	static function get_order_fields() {
		...
		$order_saved_settings = ainsys_html::get_saved_entiti_settings_from_db( ' WHERE entiti="order" AND setting_key="extra_field"', false );
		$order_extra_fields   = [];
		...
		$prepered_fields = array_merge( $prepered_fields, self::generate_acf_fields_for_entity( $order_extra_fields ) );
	...
	static function get_acf_fields() {
		...
		return self::generate_acf_fields_for_entity( $acf_group );
	}
