...
class ainsys_html{
    ...
    static function generate_entities_html(){
        ...
        $entitis_list = ainsys_settings::get_entitis();

        $properties = ainsys_settings::get_entities_settings();
        ...
        foreach ( $entitis_list as $entiti => $title ){
            ...
            $properties = ainsys_settings::get_entities_settings($entiti);

            $entities_html .= '<div class="entities_block">';

            $get_fields_function = 'get_' . $entiti . '_fields';
            $section_fields = ainsys_settings::$get_fields_function();
            ...
            if (! empty($section_fields) ){
                ...
                foreach ( $section_fields as $field_slug => $field_content ){
                    ...
                    $entiti_saved_settings = array_merge( $field_content, self::get_saved_entiti_settings_from_db(' WHERE entiti="' . $entiti . '" AND setting_name="' . $field_slug . '"') );
                    ...
                    if (!empty($field_content["children"])){
                        ...
                        foreach ($field_content["children"] as $inner_field_slug => $inner_field_content ){
                            ...
                            $entiti_saved_settings = array_merge( $field_content, self::get_saved_entiti_settings_from_db(' WHERE entiti="' . $entiti . '" AND setting_name="' . $field_slug_inner . '"') );
    ...
    static function generate_log_html( $where = '' ){
        ...
        $log_html_header = '';
        $query = "SELECT * 
        FROM " . $wpdb->prefix . ainsys_settings::$ainsys_log_table . $where;
